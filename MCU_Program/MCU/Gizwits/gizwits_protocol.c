...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagMODEL)
    {
        info->event[info->num] = EVENT_MODEL;
        info->num++;
        dataPoints->valueMODEL = gizStandardDecompressionValue(MODEL_BYTEOFFSET,MODEL_BITOFFSET,MODEL_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagAlarm)
    {
        info->event[info->num] = EVENT_Alarm;
        info->num++;
        dataPoints->valueAlarm = gizStandardDecompressionValue(Alarm_BYTEOFFSET,Alarm_BITOFFSET,Alarm_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueAlarm != cur->valueAlarm)
    {
        GIZWITS_LOG("valueAlarm Changed\n");
        ret = 1;
    }
    if(last->valueMQ_2 != cur->valueMQ_2)
    {
        GIZWITS_LOG("valueMQ_2 Changed\n");
        ret = 1;
    }
    if(last->valueMENCI_1 != cur->valueMENCI_1)
    {
        GIZWITS_LOG("valueMENCI_1 Changed\n");
        ret = 1;
    }
    if(last->valueflame_1 != cur->valueflame_1)
    {
        GIZWITS_LOG("valueflame_1 Changed\n");
        ret = 1;
    }
    if(last->valueMENCI_2 != cur->valueMENCI_2)
    {
        GIZWITS_LOG("valueMENCI_2 Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(MODEL_BYTEOFFSET,MODEL_BITOFFSET,MODEL_LEN,(uint8_t *)devStatusPtr,dataPoints->valueMODEL);
    if(dataPoints->valueAlarm >= Alarm_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valueAlarm Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(Alarm_BYTEOFFSET,Alarm_BITOFFSET,Alarm_LEN,(uint8_t *)devStatusPtr,dataPoints->valueAlarm);  
    }
    gizStandardCompressValue(MQ_2_BYTEOFFSET,MQ_2_BITOFFSET,MQ_2_LEN,(uint8_t *)devStatusPtr,dataPoints->valueMQ_2);
    gizStandardCompressValue(MENCI_1_BYTEOFFSET,MENCI_1_BITOFFSET,MENCI_1_LEN,(uint8_t *)devStatusPtr,dataPoints->valueMENCI_1);
    gizStandardCompressValue(flame_1_BYTEOFFSET,flame_1_BITOFFSET,flame_1_LEN,(uint8_t *)devStatusPtr,dataPoints->valueflame_1);
    gizStandardCompressValue(MENCI_2_BYTEOFFSET,MENCI_2_BITOFFSET,MENCI_2_LEN,(uint8_t *)devStatusPtr,dataPoints->valueMENCI_2);
